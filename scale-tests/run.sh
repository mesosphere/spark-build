...
readonly REPOSITORY_ROOT="$(cd ${SCRIPT_DIRECTORY} >/dev/null && pwd)"
...
echo "NON_GPU_QUOTA_DRIVERS_CPUS:   ${NON_GPU_QUOTA_DRIVERS_CPUS}"
echo "NON_GPU_QUOTA_DRIVERS_MEM:    ${NON_GPU_QUOTA_DRIVERS_MEM}"
echo "NON_GPU_QUOTA_EXECUTORS_CPUS: ${NON_GPU_QUOTA_EXECUTORS_CPUS}"
echo "NON_GPU_QUOTA_EXECUTORS_MEM:  ${NON_GPU_QUOTA_EXECUTORS_MEM}"
echo "NON_GPU_QUOTA_DRIVERS_MEM:    ${NON_GPU_QUOTA_DRIVERS_MEM}"
echo
echo "GPU_NUM_DISPATCHERS:          ${GPU_NUM_DISPATCHERS}"
echo "GPU_QUOTA_DRIVERS_CPUS:       ${GPU_QUOTA_DRIVERS_CPUS}"
echo "GPU_QUOTA_DRIVERS_MEM:        ${GPU_QUOTA_DRIVERS_MEM}"
echo "GPU_QUOTA_EXECUTORS_CPUS:     ${GPU_QUOTA_EXECUTORS_CPUS}"
echo "GPU_QUOTA_EXECUTORS_MEM:      ${GPU_QUOTA_EXECUTORS_MEM}"
echo "GPU_QUOTA_EXECUTORS_GPUS:     ${GPU_QUOTA_EXECUTORS_GPUS}"
echo
...
readonly NON_GPU_TOTAL_QUOTA_EXECUTORS_CPUS=$((NON_GPU_QUOTA_EXECUTORS_CPUS * STREAMING_NUM_JOBS))
readonly NON_GPU_TOTAL_QUOTA_EXECUTORS_MEM=$((NON_GPU_QUOTA_EXECUTORS_MEM * STREAMING_NUM_JOBS))

echo "NON_GPU_TOTAL_QUOTA_DRIVERS_CPUS:   ${NON_GPU_TOTAL_QUOTA_DRIVERS_CPUS}"
echo "NON_GPU_TOTAL_QUOTA_DRIVERS_MEM:    ${NON_GPU_TOTAL_QUOTA_DRIVERS_MEM}"
echo "NON_GPU_TOTAL_QUOTA_EXECUTORS_CPUS: ${NON_GPU_TOTAL_QUOTA_EXECUTORS_CPUS}"
echo "NON_GPU_TOTAL_QUOTA_EXECUTORS_MEM:  ${NON_GPU_TOTAL_QUOTA_EXECUTORS_MEM}"
...
readonly GPU_TOTAL_QUOTA_EXECUTORS_CPUS=$((GPU_QUOTA_EXECUTORS_CPUS * 5))
readonly GPU_TOTAL_QUOTA_EXECUTORS_MEM=$((GPU_QUOTA_EXECUTORS_MEM * 5))

echo "GPU_TOTAL_QUOTA_DRIVERS_CPUS:       ${GPU_TOTAL_QUOTA_DRIVERS_CPUS}"
echo "GPU_TOTAL_QUOTA_DRIVERS_MEM:        ${GPU_TOTAL_QUOTA_DRIVERS_MEM}"
echo "GPU_TOTAL_QUOTA_EXECUTORS_CPUS:     ${GPU_TOTAL_QUOTA_EXECUTORS_CPUS}"
echo "GPU_TOTAL_QUOTA_EXECUTORS_MEM:      ${GPU_TOTAL_QUOTA_EXECUTORS_MEM}"
...
log "[ ${container_running} -ne 0 ]"
if [ ${container_running} -ne 0 ]; then
  log "Container '${CONTAINER_NAME}' already running"
fi

log "[ ${container_finished_setting_up} -ne 0 ]"
if [ ${container_finished_setting_up} -ne 0 ]; then
  log "Container '${CONTAINER_NAME}' didn't finish setting up"
fi

log "[ ${container_running} -ne 0 ] || [ ${container_finished_setting_up} -ne 0 ]"
if [ ${container_running} -ne 0 ] || [ ${container_finished_setting_up} -ne 0 ]; then
  log "something is wrong, rebuild the image"
fi
