...
def create_quota(role_name: str, quota: typing.Dict):
    ...
    """
    existing_quotas = sdk_cmd.get_json_output(
        "spark quota list --json", print_output=False
    )
...
def install_package(
    ...
    setup_spark_security(
        service_name, group_role, None, None, service_account_info
    )
...
def get_default_options(arguments: dict) -> dict:
    ...
    options = {
        "service": {
            ...
            "UCR_containerizer": ast.literal_eval(
                arguments.get("--ucr-containerizer", True)
            ),
