...
def submit_job(
    ...
    log.info(
        "Submitting job to dispatcher: %s, with duration: %s min.",
        dispatcher_name,
        duration,
    )

    # TODO: use group role if enforce_role is enabled.
...
def submit_loop(
    ...
    while True:
        ...
        if app_url == MONTE_CARLO_APP_URL:
            app_args = "100000 {}".format(
                str(duration * 30)
            )  # about 30 iterations per min.
        else:
