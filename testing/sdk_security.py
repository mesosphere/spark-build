...
def install_enterprise_cli(force=False):
    ...
    if not force:
        ...
        if stdout:
            log.info(
                "DC/OS enterprise version %s CLI already installed", stdout.strip()
            )
            return
    ...
    try:
        ...
        raise RuntimeError(
            "Failed to install the dcos-enterprise-cli: {}".format(repr(e))
        )
...
def _grant(user: str, acl: str, description: str, action: str = "create") -> None:
    ...
    assert r.status_code in [201, 409], "{} failed {}: {}".format(
    ...
    r = sdk_cmd.cluster_request(
        "PUT",
        "/acs/api/v1/acls/{acl}/users/{user}/{action}".format(
            acl=acl, user=user, action=action
        ),
        raise_on_error=False,
    )
    # 204=success, 409=already exists
    assert r.status_code in [204, 409], "{} failed {}: {}".format(
        r.url, r.status_code, r.text
    )
...
def get_permissions(service_account_name: str, linux_user: str) -> typing.List[dict]:
    return [
        ...
        {
            "user": service_account_name,
            "acl": "dcos:mesos:master:reservation:principal:{}".format(
...
def grant_permissions(
    ...
    log.info(
        "Permission setup completed for {account}".format(account=service_account_name)
    )


def revoke_permissions(
    linux_user: str, role_name: str, service_account_name: str
) -> None:
    ...
    log.info(
        "Permission cleanup completed for {account}".format(
...
def create_service_account(
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        sdk_cmd.run_cli(
            "security org service-accounts keypair {} {}".format(
...
def openssl_ciphers():
    return set(
        check_output(["openssl", "ciphers", "ALL:eNULL"])
        .decode("utf-8")
        .rstrip()
        .split(":")
    )
